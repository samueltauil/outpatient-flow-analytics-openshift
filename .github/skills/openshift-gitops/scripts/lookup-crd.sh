...
#   ./lookup-crd.sh argocds              # Show top-level spec fields
#   ./lookup-crd.sh argocds spec.server  # Show nested field details
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "$0")/../../../.." 2>/dev/null || cd "$(git rev-parse --show-toplevel 2>/dev/null || echo .)" && pwd)"
CRD_DIR="$REPO_ROOT/openshift/crds/openshift-gitops-operator"
