...
#   ./lookup-crd.sh tektonconfigs              # Show top-level spec fields
#   ./lookup-crd.sh tektonconfigs spec.pipeline  # Show nested field details
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "$0")/../../../.." 2>/dev/null || cd "$(git rev-parse --show-toplevel 2>/dev/null || echo .)" && pwd)"
CRD_DIR="$REPO_ROOT/openshift/crds/openshift-pipelines-operator-rh"
